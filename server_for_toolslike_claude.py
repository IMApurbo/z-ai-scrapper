...
from zai_scraper import ZAIScraper
...
def _messages_to_prompt(messages: list) -> str:
    ...
    for msg in messages:
        ...
        if isinstance(content, str):
            ...
            # Extract text blocks only (images/tools not supported in z.ai)
            text_parts = []
            for block in content:
                if isinstance(block, dict):
                    ...
                    if btype == "text":
                        ...
                        # Flatten tool results as plain text
    ...
    # Append the assistant turn-starter so z.ai continues naturally
...
def _build_response_body(content_text: str, model: str, usage_in: int = 0) -> dict:
    ...
    usage_out = len(content_text.split())  # rough token estimate
    return {
...
def _stream_response(content_text: str, model: str):
    ...
    # Stream in ~40-char chunks so the client sees progressive output
    chunk_size = 40
...
def _auth_check():
    """
    Accept any Bearer token — we don't validate it, just ensure the header
    exists so Claude Code / SDK don't bail out immediately.
    """
    # Skip OPTIONS (CORS preflight)
    if request.method == "OPTIONS":
        return _cors_response()
    # Allow through — key validation is handled by z.ai login, not us
...
def messages():
    ...
    # For simple single-turn requests, just send the last user message directly
    # For multi-turn, we inject the full history as a structured prompt
    ...
    try:
        scraper = get_scraper()
        md, html, was_web = scraper.send_message(prompt)
...
if __name__ == "__main__":
    ...
    # Optionally warm up the browser before the first request
    if not args.no_warmup:
        print("[*] Pre-launching browser (--no-warmup to skip)...")

        # Patch headless flag before init
        import zai_scraper as _zs
